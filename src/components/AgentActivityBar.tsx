...
const activityTemplates = [
  { agentId: "vlad", action: "Pushed 3 commits to", target: "feat/collab-editing" },
  { agentId: "agnes", action: "Updated design tokens for", target: "dark mode system" },
  { agentId: "ivar", action: "Updated sprint board with", target: "velocity insights" },
  { agentId: "bjorn", action: "Health check passed for", target: "EU-West deploy" },
  { agentId: "agnes", action: "Finalized onboarding mockups for", target: "NEXUS-202" },
  { agentId: "vlad", action: "Resolved merge conflict in", target: "collaboration.ts" },
  { agentId: "ivar", action: "Created sub-task for", target: "NEXUS-191" },
  { agentId: "bjorn", action: "Scaled up instances for", target: "US-East region" },
  { agentId: "vlad", action: "Refactored auth middleware in", target: "server/auth.go" },
  { agentId: "agnes", action: "Exported icon set for", target: "component library" },
  { agentId: "ivar", action: "Moved ticket to review:", target: "NEXUS-178" },
  { agentId: "bjorn", action: "SSL cert renewed for", target: "api.nexus.dev" },
  { agentId: "vlad", action: "Added unit tests for", target: "useCollabHook.ts" },
  { agentId: "agnes", action: "Created motion spec for", target: "page transitions" },
  { agentId: "ivar", action: "Assigned code review to", target: "Vlad & Agnes" },
  { agentId: "bjorn", action: "Deployed hotfix to", target: "production-v2.4.1" },
];
...
export default function AgentActivityBar({ onCollapse }: { onCollapse?: () => void }) {
  ...
  const templateIdx = useRef(6);

  useEffect(() => {
    const interval = setInterval(() => {
      const template = activityTemplates[templateIdx.current % activityTemplates.length];
      templateIdx.current++;
  ...
  return (
    <div className="w-72 bg-card border-l border-border flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto relative">
        ...
        <div className="px-3 space-y-0.5 pb-3 pt-1">
          {entries.map((entry) => {
            const agent = agents.find((a) => a.id === entry.agentId)!;
            return (
              <div
                key={entry.id}
                className="flex gap-2.5 px-2 py-2 hover:bg-accent/50 transition-all duration-500 animate-slide-in-activity"
              >
                <img
                  src={agent.avatar}
                  alt={agent.name}
